...
public class WhiteboardHandlerUtil {
	...
	private static final Log LOG = LogFactoryUtil
			.getLog(WhiteboardHandlerUtil.class);
	 ...
	 * @return
	 */
	public static JSONObject generateLoggedUsersJSON(
			ConcurrentMap<String, UserData> loggedUserMap) {
	 ...
	 * @param jsonMessage
	 */
	public static void persistWhiteboardDump(
		...
		if (jsonMessage.getJSONArray(COMMANDS) != null) {
			...
			for (int i = 0; i < commandsLength; i++) {
				...
				if (!command.getString(ACTION).equals(USERS)
						&& !command.getString(ACTION).equals(TOOLTIP)) {
					...
					if (command.getString(ACTION).equals(DELETE)) {
						...
						if (whiteBoardDump.get(command.getString(CACHEID)) != null) {
							JSONObject cachedCommand = whiteBoardDump
									.get(command.getString(CACHEID));
							if (command.getString(TYPE).equals(LINE)) {

								cachedCommand.getJSONObject(STATE).put(OPTIONS,
										command.getJSONObject(STATE));

							} else if (command.getString(TYPE).equals(CIRCLE)) {

								cachedCommand.put(STATE,
										command.getJSONObject(STATE));
								cachedCommand.getJSONObject(STATE).put(RADIUS,
										CIRCLE_RADIUS);
	 ...
	 * @return
	 */
	public static JSONArray loadWhiteboardDump(
			ConcurrentMap<String, JSONObject> whiteBoardDump) {
